...
def generar_calendario(fecha, size=80):
    ...
    try:
        font_mes = ImageFont.truetype(
            "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",
            int(size * 0.3)
        )
        font_dia = ImageFont.truetype(
            "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",
            int(size * 0.65)
        )
    except:
    ...
    # Medir texto (compatibilidad Pillow viejo/nuevo)
    try:
        # Pillow <10
        ...
        # Pillow >=10
        w_mes = draw.textbbox((0, 0), mes, font=font_mes)[2]
        w_dia = draw.textbbox((0, 0), dia, font=font_dia)[2]

    # Mes en blanco sobre rojo
